...
class SemaDataResources
{
    ...
    #[McpResource(
        uri: 'xcart://sema/categories',
        name: 'sema_category_tree',
        title: 'SEMA Data Category Tree',
        description: 'SEMA Data imported category tree with mapping status to X-Cart categories.',
    ...
    #[McpResource(
        uri: 'xcart://sema/categories/unmapped',
        name: 'sema_unmapped_categories',
        title: 'SEMA Data Unmapped Categories',
        description: 'SEMA Data categories not yet mapped to X-Cart categories',
        mimeType: 'application/json'
    )]
    public function getUnmappedCategories(): array
    {
        ...
        $rows = $this->conn->fetchAllAssociative(
            "SELECT t.id, t.parent_id, t.remoteId, t.name,
             ...
             FROM {$this->semaTable} t
             LEFT JOIN {$this->semaTable} pt ON t.parent_id = pt.id
    ...
    #[McpResource(
        uri: 'xcart://sema/mapping-summary',
        name: 'sema_mapping_summary',
        title: 'SEMA Data Mapping Summary',
        description: 'Summary of SEMA Data to X-Cart category mapping progress with exact name match suggestions and duplicate detection.',
        mimeType: 'application/json'
    )]
    public function getMappingSummary(): array
    {
        ...
        $total = (int) $this->conn->fetchOne("SELECT COUNT(*) FROM {$this->semaTable}");
        $mapped = (int) $this->conn->fetchOne("SELECT COUNT(*) FROM {$this->semaTable} WHERE category_id IS NOT NULL");
        $roots = (int) $this->conn->fetchOne("SELECT COUNT(*) FROM {$this->semaTable} WHERE parent_id IS NULL");
        $children = (int) $this->conn->fetchOne("SELECT COUNT(*) FROM {$this->semaTable} WHERE parent_id IS NOT NULL");
        ...
        $potentialMatches = $this->conn->fetchAllAssociative(
            "SELECT t.id AS sema_id, t.name AS sema_name, c.category_id, ct.name AS xcart_name
             FROM {$this->semaTable} t
        ...
        foreach ($potentialMatches as $m) {
            $exactMatches[] = [
                'sema_id' => (int) $m['sema_id'],
                'sema_name' => $m['sema_name'],
